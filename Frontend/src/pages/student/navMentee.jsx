...
function NavMentee() {
  ...
  const handleSubmitFeedback = async () => {
    if (!feedbackMentee || feedbackRating === 0 || feedbackText.trim() === '') {
    ...
    try {
      await axios.post('http://localhost:5000/mentor/feedback', payload);
      setMentee_list(prev =>
        prev.map(m => m.mentee_email === feedbackMentee.mentee_email ? { ...m, mentorf: 'yes' } : m)
      );
                  ...
                  <div key={id} className="bg-white rounded-lg shadow-md p-4 flex flex-col justify-between">
                    <div className="flex flex-row">
                      ...
                      <div className="flex flex-col justify-center sm:ml-4">
                        ...
                        <p className="font-medium flex text-[16px] my-1">{data.language_name}<span className="pl-1">level {data.mentee_current_level}</span></p>

                        {isBooked && (
                          <div>
                            <span className={`w-fit mt-1 px-4 py-1 rounded-full font-semibold text-sm ${
                            ...
                            {data.level_cleared === 'ongoing' && (
                              <p className="text-gray-700 hover:underline mt-3 hover:cursor-pointer" onClick={() => handleViewVenue(data)}>
                                View Venue
                              </p>
                            )}
                          </div>
      ...
      {showFeedbackPopup && feedbackMentee && (
        <div className="fixed top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.5)] flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-6 w-[90%] max-w-md">
            ...
            <p><strong>Mentee:</strong> {feedbackMentee.mentee_email}</p>
            <p><strong>Language:</strong> {feedbackMentee.language_name}</p>
            ...
            <div className="flex justify-between gap-4">
              <button className="bg-gray-300 text-gray-800 py-2 px-4 rounded w-1/2" onClick={() => setShowFeedbackPopup(false)}>Cancel</button>
              <button className="bg-[#7D53F6] text-white py-2 px-4 rounded w-1/2" onClick={handleSubmitFeedback}>Submit</button>
            </div>
          </div>
        </div>
      )}
